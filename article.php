...
$additional_article_list = array(36,37,38,52,48,49,50,51,72,73,74,75,76,77,85); //妖怪类
$news_cat_list = array(20,21,22,23,17,29);
if(in_array($_GET['id'],$additional_article_list)){
	$smarty->assign('article_type', 2);
}else{
	if(in_array($article["cat_id"],$news_cat_list)){
		$smarty->assign('article_type', 3);
		// 获取其他相关资讯
		if ($article["cat_id"]==17){
			$sql = "SELECT *".
            "FROM " .$GLOBALS['ecs']->table('article').
            "WHERE is_open = 1 AND cat_id = 21 ORDER BY add_time DESC LIMIT 0,10";
		}else{
			$sql = "SELECT *".
            "FROM " .$GLOBALS['ecs']->table('article').
            "WHERE is_open = 1 AND cat_id = ".$article["cat_id"]." ORDER BY add_time DESC LIMIT 0,10";
		}
		
		$article_list10 = $GLOBALS['db']->getAll($sql);
		$smarty->assign('article_list10', $article_list10);
		// 获取企业资讯
		$sql = "SELECT *".
            "FROM " .$GLOBALS['ecs']->table('article').
            "WHERE is_open = 1 AND cat_id = 20 ORDER BY add_time DESC LIMIT 0,5";
		$article_list20 = $GLOBALS['db']->getAll($sql);	
		$smarty->assign('article_list20', $article_list20);	

		// 获取文章添加时间
		//print_r($article["add_time"]);
		$smarty->assign('formed_add_time', $article["add_time"]);	
	}else{
...
$additional_article_list1 = array(36,37,38,52);
if(in_array($_GET['id'],$additional_article_list1)){
	$smarty->assign('article_small_type', 1);
}

$additional_article_list2 = array(72,73,74,75,76,77);
if(in_array($_GET['id'],$additional_article_list2)){
	$smarty->assign('article_small_type', 2);
}

$additional_article_list3 = array(48,49,50,51,85);
if(in_array($_GET['id'],$additional_article_list3)){
	$smarty->assign('article_small_type', 3);
}
//$smarty->assign('article',$article);
$smarty->assign('article',$article);
//print_r($article);
