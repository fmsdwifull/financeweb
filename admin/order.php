...
elseif ($_REQUEST['act'] == 'info')
{
  ...
  //echo $_REQUEST["order_id"];exit;
	//$orid=$_REQUEST["order_id"];
	$sql = "SELECT u.true_name, u.user_name, u.id_number, g.goods_id, g.goods_name, g.goods_interest_rate, g.goods_use,p.project_name,p.owner_name,p.fuiou_id,s.seller_name,
				o.pay_time,(g.goods_start_time+g.goods_period*24*3600) as pay_end_time,o.*, 
				g.goods_earn_method,t FROM "
				.$GLOBALS["ecs"]->table("order_new") ." AS o 
	...
	$earn_list=array();
	$pay_end_time=date('Y年m月d日 H点i分s秒',$order_info['pay_end_time']);//到期兑付日
	print_r($pay_end_time);
	
	if($order_info['transfer_flag'] == 2 && !$order_info['parent_id'] ){
		//print_r("123");
		//已转让订单  不是从别人那转让那边买来的
		$pay_time=date('Y年m月d日 H点i分s秒',$order_info['pay_time']);
		$left_time=$order_info['transfer_end_time']-$order_info['pay_time']-$order_info['t'];
		$left_time=floor($left_time/3600/24);//剩多少天
		$get_paid=0;
		
		if($order_info['goods_earn_method']==2){
				//到期一次还回购
				$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*$left_time);
				$principal_list=array('principal'=>$principal,'time'=>date('Y年m月d日 H点i分s秒',$order_info['transfer_end_time']));		
				
		}elseif($order_info['goods_earn_method']==3){
				//30天还一次
				
				if($left_time%30==0){
					
					$count=floor($left_time/30);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*30);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$order_info['pay_time']+(30*$i+$order_info['t'])*3600*24));	
						$earn_amount+=$earn_money;		
					}	
					
				}else{
					
					for($i=1;$i<floor($left_time/30);$i++){
						$get_paid++;		
					}			

					$left_time=floor(($order_info['transfer_end_time']-$get_paid*30*3600*24-$order_info['pay_time'])/3600/24);
					$count=floor($left_time/30);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*30);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$get_paid*30*3600*24+$order_info['pay_time']+(30*$i+$order_info['t'])*3600*24));	
						$earn_amount+=$earn_money;		
					}	
				}	
				if($left_time-30*$count>0){
					$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*($left_time-30*$count)+$order_info['amount']-$order_info['transfer_amount']);
					$principal_list=array('principal'=>$principal.'元','time'=>date('Y年m月d日 H点i分s秒',$order_info['transfer_end_time']));	
				}
				
			
		}elseif($order_info['goods_earn_method']==4){
				//90天还一次
				//print_r("1");
				if($left_time%90==0){
					//print_r("2");
					$count=floor($left_time/90);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*90);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日 ',$order_info['pay_time']+(90*$i+$order_info['t'])*3600*24));	
						$earn_amount+=$earn_money;		
					}	
					
				}else{
					
					for($i=1;$i<floor($left_time/90);$i++){
						$get_paid++;		
					}			

					$left_time=floor(($order_info['transfer_end_time']-$get_paid*90*3600*24-$order_info['pay_time'])/3600/24);
					$count=floor($left_time/90);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*90);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$get_paid*90*3600*24+$order_info['pay_time']+(90*$i+$order_info['t'])*3600*24));	
						$earn_amount+=$earn_money;		
					}	
				}	
				if($left_time-90*$count>0){
					$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*($left_time-90*$count)+$order_info['amount']-$order_info['transfer_amount']);
					$principal_list=array('principal'=>$principal.'元','time'=>date('Y年m月d日 H点i分s秒',$order_info['transfer_end_time']));	
				}		
	...
	elseif($order_info['transfer_flag'] == 2 && $order_info['parent_id'] ){
		//已转让订单  从别人那转让那边买来的
		//print_r("ok");
		$pay_time=date('Y年m月d日 H点i分s秒',$order_info['transfer_time']);
		$left_time=$order_info['transfer_end_time']-$order_info['transfer_time'];
		$left_time=floor($left_time/3600/24);//剩多少天		
		$get_paid=0;
		...
		if($order_info['goods_earn_method']==2){
				...
				if($left_time%30==0){
					...
					$left_time=floor(($order_info['transfer_end_time']-$get_paid*30*3600*24-$order_info['transfer_time'])/3600/24);
					$count=floor($left_time/30);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*30);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$get_paid*30*3600*24+$order_info['pay_time']+(30*$i+$order_info['t'])*3600*24));	
				...
				if($left_time%90==0){
					...
					$left_time=floor(($order_info['transfer_end_time']-$get_paid*90*3600*24-$order_info['transfer_time'])/3600/24);
					$count=floor($left_time/90);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*90);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$get_paid*90*3600*24+$order_info['pay_time']+(90*$i+$order_info['t'])*3600*24));	
		...
		//print_r("56");
		//未转让的订单 transfer_flag==0  || ==1  ok！
		if($order_info['parent_id']){
			//从别人那转让那边买来的 未转让给别人的
			//print_r("56");
			$pay_time=date('Y年m月d日 H点i分s秒',$order_info['transfer_time']);//转入时间
			$left_time=$order_info['pay_end_time']-$order_info['transfer_time'];
			$left_time1=$order_info['transfer_time']-$order_info['pay_time'];
			
			$left_time=floor($left_time/3600/24);//剩多少天
			$left_time1=floor($left_time1/3600/24);//剩多少天
			$get_paid=0;
			...
			if($order_info['goods_earn_method']==2){
				//到期一次还回购
				$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*$left_time);
				$principal_list=array('principal'=>$principal,'time'=>date('Y年m月d日',$order_info['pay_end_time']));		
				
			}elseif($order_info['goods_earn_method']==3){
				//30天还一次
				if($left_time1%30==0){
					
					$count=floor($left_time1/30);
					for($i=1;$i<=$count;$i++){
						$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*30);
						$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$order_info['transfer_time']+(30*$i)*3600*24));	
						$earn_amount+=$earn_money;		
					}	
					
				}else{
					
					for($i=1;$i<floor($left_time1/30);$i++){
						$get_paid++;		
					}			

					$left_time1=floor(($order_info['pay_end_time']-$get_paid*30*3600*24-$order_info['transfer_time'])/3600/24);
					$count=floor($left_time1/30);
				...
				if($left_time1-30*$count>0){
					$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*($left_time1-30*$count)+$order_info['amount']);
					$principal_list=array('principal'=>$principal.'元','time'=>date('Y年m月d日',$order_info['pay_end_time']));	
				}
				...
				//90天还一次
					if($left_time1%90==0){
						...
						$count=floor($left_time1/90);
						for($i=1;$i<=$count;$i++){
							$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*90);
							$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$order_info['transfer_time']+(90*$i)*3600*24));	
							$earn_amount+=$earn_money;		
						}	
						
					}else{
						
						for($i=1;$i<floor($left_time1/90);$i++){
							$get_paid++;		
						}			
						$left_time1=floor(($order_info['pay_end_time']-$get_paid*90*3600*24)/3600/24);
						$count=floor($left_time1/90);
						for($i=1;$i<=$count;$i++){
							$earn_money=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*90);
							$earn_list[]=array('interest'=>$earn_money.'元','time'=>date('Y年m月d日',$order_info['pay_time']+(90*$i+$order_info['t'])*3600*24));	
					...
					if($left_time1-90*$count>0){
						$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*($left_time1-90*$count)+$order_info['amount']);
						$principal_list=array('principal'=>$principal.'元','time'=>date('Y年m月d日',$order_info['pay_end_time']));	
					}
			...
			//修改
			$pay_time=date('Y年m月d日 H点i分s秒',$order_info['pay_time']);
			$left_time=$order_info['pay_end_time']-$order_info['pay_time']-$order_info['t'];
			$left_time=floor($left_time/3600/24);//剩多少天
			
			if($order_info['goods_earn_method']==1){
				//到期一次性兑付
				$principal=sprintf('%.2f', ($order_info['goods_interest_rate']/365/100*$order_info['amount']*$left_time+$order_info['amount']));
				$principal_list=array('principal'=>$principal,'time'=>date('Y年m月d日',$order_info['pay_end_time']));
				//print_r
			}elseif($order_info['goods_earn_method']==2){
			//到期一次还回购
				$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*$left_time);
				$principal_list=array('principal'=>$principal,'time'=>date('Y年m月d日',$order_info['pay_end_time']));			
			}elseif($order_info['goods_earn_method']==3){
				...
				if($left_time-30*$count>0){
					$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*($left_time-30*$count)+$order_info['amount']);
					$principal_list=array('principal'=>$principal.'元','time'=>date('Y年m月d日',$order_info['pay_end_time']));	
				}
				...
				if($left_time-90*$count>0){
					$principal=sprintf('%.2f',$order_info['goods_interest_rate']/365/100*$order_info['amount']*($left_time-90*$count)+$order_info['amount']);
					$principal_list=array('principal'=>$principal.'元','time'=>date('Y年m月d日',$order_info['pay_end_time']));	
				}		
			}
	...
	$order_info['interest_amount']=$earn_amount+$principal.'元';	//总利息

	$smarty->assign("pay_time",$pay_time);
	$smarty->assign("pay_end_time",$pay_end_time);
	$smarty->assign("order_info",$order_info);
	$smarty->assign("earn_list",$earn_list);
	$smarty->assign("principal_list",$principal_list);
...
function order_list()
{
        ...
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_new') . " AS o ". $where;
        }
        ...
        /* 查询 */
        $sql = "SELECT o.*,u.*,g.goods_name,s.seller_name " .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['short_order_time'] = date('Y-m-d H:i:s', $value['pay_time']);
     
		if($value['transfer_flag']==2){
			//已转让
			   $row[$key]['transfer_detail'] = '已转让';
		}elseif($value['transfer_flag']==1){
				$row[$key]['transfer_detail'] = '转让中';
		}else{
				$row[$key]['transfer_detail'] = '未转让';	
		}
    }
