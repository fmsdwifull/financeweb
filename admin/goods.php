...
//print_r(dirname(__FILE__) . '/includes/init.php');exit();
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	$sql = "SELECT project_id, project_name FROM ".$GLOBALS["ecs"]->table("project");
    ...
    if ($is_add)
    {
        ...
        $goods = array(
			...
			'goods_rest_number'  => 0,
			'goods_interest_rate'=> 0,
			'goods_period'       => 0,
			'goods_min_buy'      => 0,
			'goods_people_number'=> 200,
			'goods_earn_method'  => 0,
			'goods_level'		 => '',
			'goods_start_year'   => 0, 'goods_start_month'   => 0, 'goods_start_day'   => 0,
			'goods_start_hour'   => 0, 'goods_start_minute'   => 0, 'goods_start_second'   => 0,
			'goods_end_year'   => 0, 'goods_end_month'   => 0, 'goods_end_day'   => 0,
			'goods_end_hour'   => 0, 'goods_end_minute'   => 0, 'goods_end_second'   => 0,
			'goods_earn_start'	 => 0,
			'goods_max_buy'		 => '',
			'goods_caracter'	 => '',
			'goods_garentee'	 => '',
			'goods_sale_time_start'		 => '10:00',
			'goods_sale_time_end'		 => '16:30',
			'goods_object'		 => '',
			'goods_basic_asset'	 => '',
			'goods_use'		 => '',
			'goods_beizhu'		 => '',
    ...
    else
    {
		...
		$goods['goods_start_year'] = date('Y', $goods['goods_start_time']);
		$goods['goods_start_month'] = date('m', $goods['goods_start_time']);
		$goods['goods_start_day'] = date('d', $goods['goods_start_time']);
		$goods['goods_start_hour'] = date('H', $goods['goods_start_time']);
		$goods['goods_start_minute'] = date('i', $goods['goods_start_time']);
		$goods['goods_start_second'] = date('s', $goods['goods_start_time']);
		
		$goods['goods_end_year'] = date('Y', $goods['goods_end_time']);
		$goods['goods_end_month'] = date('m', $goods['goods_end_time']);
		$goods['goods_end_day'] = date('d', $goods['goods_end_time']);
		$goods['goods_end_hour'] = date('H', $goods['goods_end_time']);
		$goods['goods_end_minute'] = date('i', $goods['goods_end_time']);
		$goods['goods_end_second'] = date('s', $goods['goods_end_time']);
        /* 获取商品类型存在规格的类型 */
        $specifications = get_goods_type_specifications();
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	/* 通过$project_id 得到富有账号等 */
	
	$sql = "SELECT fuiou_id, owner_name, owner_licence, owner_code  FROM " . $ecs->table('project') . " WHERE project_id = '$project_id'";
    
	$array_add = $db->getrow($sql);
	
	//print_r($array);
	$fuiou_idn = $array_add['fuiou_id'];
	$owner_name = $array_add['owner_name'];
	$owner_lin = $array_add['owner_licence'];
	$owner_con = $array_add['owner_code'];
	
	//富有账号：
	$goods_fuiou_id = !empty($_POST['fuiou_id']) ? $_POST['fuiou_id'] : $fuiou_idn;
	//print_r($goods_fuiou_id);
	$goods_serie = !empty($_POST['goods_serie']) ? $_POST['goods_serie'] : '';
	$goods_owner_name = !empty($_POST['goods_owner_name']) ? $_POST['goods_owner_name'] : $owner_name;
	
	$goods_owner_licence = !empty($_POST['goods_owner_licence']) ? $_POST['goods_owner_licence'] : $owner_lin;
	$goods_owner_code = !empty($_POST['goods_owner_code']) ? $_POST['goods_owner_code'] : $owner_con;
	
	$goods_transfer_flag = !empty($_POST['goods_transfer_flag']) ? $_POST['goods_transfer_flag'] : 0;
	$goods_use = !empty($_POST['use']) ? $_POST['use'] : '';
	$goods_start_year = !empty($_POST['start_year']) ? $_POST['start_year'] : 1970;
	$goods_start_month = !empty($_POST['start_month']) ? $_POST['start_month'] : 1;
	$goods_start_day = !empty($_POST['start_day']) ? $_POST['start_day'] : 1;
	$goods_start_hour = !empty($_POST['start_hour']) ? $_POST['start_hour'] : 0;
	$goods_start_minute = !empty($_POST['start_minute']) ? $_POST['start_minute'] : 0;
	$goods_start_second = !empty($_POST['start_second']) ? $_POST['start_second'] : 0;
	$goods_end_year = !empty($_POST['end_year']) ? $_POST['end_year'] : 1970;
	$goods_end_month = !empty($_POST['end_month']) ? $_POST['end_month'] : 1;
	$goods_end_day = !empty($_POST['end_day']) ? $_POST['end_day'] : 1;
	$goods_end_hour = !empty($_POST['end_hour']) ? $_POST['end_hour'] : 0;
	$goods_end_minute = !empty($_POST['end_minute']) ? $_POST['end_minute'] : 0;
	$goods_end_second = !empty($_POST['end_second']) ? $_POST['end_second'] : 0;
	$goods_start_time = mktime($goods_start_hour,$goods_start_minute,$goods_start_second,$goods_start_month,$goods_start_day,$goods_start_year);
	$goods_end_time = mktime($goods_end_hour,$goods_end_minute,$goods_end_second,$goods_end_month,$goods_end_day,$goods_end_year);	
	$goods_earn_start = !empty($_POST['earn_start']) ? $_POST['earn_start'] : 1;
	$goods_max_buy    = !empty($_POST['max_buy']) ? $_POST['max_buy'] : $_POST['total_number'];
	$goods_caracter   = !empty($_POST['caracter']) ? $_POST['caracter'] : 1;
	$goods_garentee  = !empty($_POST['goods_garentee']) ? $_POST['goods_garentee'] : 1;
	$goods_sale_time_start = !empty($_POST['sale_time_start']) ? $_POST['sale_time_start'] : 1;
	$goods_sale_time_end   = !empty($_POST['sale_time_end']) ? $_POST['sale_time_end'] : 1;
	$goods_object     = !empty($_POST['object']) ? $_POST['object'] : 1;
	$goods_basic_asset= !empty($_POST['basic_asset']) ? $_POST['basic_asset'] : "";
	$goods_beizhu     = !empty($_POST['beizhu']) ? $_POST['beizhu'] : "";
	$goods_earn_method = !empty($_POST['earn_method']) ? $_POST['earn_method'] : 1;
	$goods_level = !empty($_POST['level']) ? $_POST['level'] : '';
	$goods_period = !empty($_POST['period']) ? $_POST['period'] : 365;
	$goods_min_buy = !empty($_POST['min_buy']) ? $_POST['min_buy'] : 100;
	$goods_people_number = !empty($_POST['people_number']) ? $_POST['people_number'] : 100;
	$goods_total_number = !empty($_POST['total_number']) ? $_POST['total_number'] : 0;
	$goods_rest_number = !empty($_POST['rest_number']) ? $_POST['rest_number'] : 0;
	$goods_interest_rate = !empty($_POST['interest_rate']) ? $_POST['interest_rate'] : 0;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name,project_id,t,fuiou_id,goods_serie, goods_owner_name, goods_owner_licence, goods_owner_code, goods_transfer_flag, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, goods_earn_method,goods_level,goods_period,goods_min_buy,goods_people_number,goods_start_time,goods_end_time, goods_earn_start, goods_max_buy, goods_caracter, goods_garentee, goods_sale_time_start, goods_sale_time_end, goods_object, goods_basic_asset, goods_use, goods_beizhu,   goods_total_number,goods_rest_number,goods_interest_rate, shop_price, market_price, is_promote, promote_price, " .
                ...
                "VALUES ('$_POST[goods_name]','$project_id','$t','$goods_fuiou_id','$goods_serie', '$goods_owner_name','$goods_owner_licence','$goods_owner_code','$goods_transfer_flag','$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id','$goods_earn_method','$goods_level','$goods_period','$goods_min_buy','$goods_people_number','$goods_start_time','$goods_end_time','$goods_earn_start','$goods_max_buy','$goods_caracter','$goods_garentee','$goods_sale_time_start','$goods_sale_time_end','$goods_object','$goods_basic_asset','$goods_use','$goods_beizhu','$goods_total_number','$goods_rest_number','$goods_interest_rate', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
				...
				"t = '$_POST[t]', ".
				"fuiou_id = '$goods_fuiou_id', " .
				"goods_serie = '$_POST[goods_serie]', " .
				"goods_owner_name = '$owner_name', " .
				"goods_owner_licence = '$goods_owner_licence', " .
				"goods_owner_code = '$goods_owner_code', " .
				"goods_transfer_flag = '$_POST[goods_transfer_flag]', " .
                "goods_name_style = '$goods_name_style', " .
                "goods_sn = '$goods_sn', " .
                "cat_id = '$catgory_id', " .
                "brand_id = '$brand_id', " .
				"goods_earn_start = '$goods_earn_start', " .
				"goods_max_buy = '$goods_max_buy', " .
				"goods_caracter = '$goods_caracter', " .
				"goods_garentee = '$goods_garentee', " .
				"goods_sale_time_start = '$goods_sale_time_start', " .
				"goods_sale_time_end = '$goods_sale_time_end', " .
				"goods_object = '$goods_object', " .
				"goods_basic_asset = '$goods_basic_asset', " .
				"goods_use = '$goods_use', " .	
				"goods_beizhu = '$goods_beizhu', " .	
				"goods_earn_method = '$goods_earn_method', " .
				"goods_level = '$goods_level', " .				
				"goods_period = '$goods_period', " .
				"goods_min_buy = '$goods_min_buy', " .
				"goods_people_number = '$goods_people_number', " .
				"goods_start_time = '$goods_start_time', " .
				"goods_end_time = '$goods_end_time', " .
				"goods_total_number = '$goods_total_number', " . 
				"goods_rest_number = '$goods_rest_number', " . 
				"goods_interest_rate = '$goods_interest_rate', " . 
