...
?>
  <tr  style="<?php if ($this->_var['item']['is_cancel']): ?>color:#ddd<?php endif; ?>">
	...
	<?php if (! $this->_var['item']['is_cancel']): ?>
		...
		<?php else: ?>
		<form method="POST" action="user_reserve.php?act=submit_seller" style="margin-bottom:0">
...
?>
	...
	<?php else: ?>
	<form action="user_reserve.php?act=submit_time" method = "POST"style="margin-bottom:0" id="form_<?php echo $this->_var['item']['reserve_id']; ?>">
		<td>
...
<form action="user_reserve.php?act=logout" method="POST">
<input type="submit" value="登出">
</form>
<?php else: ?>
<form action="user_reserve.php?act=login" method="POST">
请输入业务员序列号：
<input type="password" name="seller_no">
<input type="submit">
</form>
<?php endif; ?>
<?php if ($this->_var['full_page']): ?>
