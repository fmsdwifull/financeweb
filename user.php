...
$not_login_arr =
array('login','act_login','register','act_register','act_get_password_m','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'recharge','withdraw','reserve_log','money_log', 'user_product', 'identify','bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
//预约记录
if ($action == 'reserve_log')
{
	$sql = 'SELECT r.reserve_id, g.goods_name,s.store_name, ser.seller_name,r.is_come,r.is_cancel, r.year, r.month, r.day, r.hour, r.minute FROM ' . $GLOBALS['ecs']->table('reserve') . " AS r 
			INNER JOIN " . $ecs->table("store") . " AS s ON s.code=r.point
			INNER JOIN " . $ecs->table("seller") . " AS ser ON ser.seller_id=r.seller_id
			INNER JOIN " . $ecs->table("goods") . " AS g ON g.goods_id=r.goods_id
			WHERE r.user_id = ".$_SESSION["user_id"];
	$reserve_list = $GLOBALS['db']->getAll($sql);
	$smarty->assign('reserve_list', $reserve_list);	
...
elseif ($action == 'default')
{
	...
	if($_GET['login_status']==1){
		//print_r($_GET['login_status']);
		Header("Location:category-5-3-0-0.html");
	}elseif($_GET['login_status']==2){
		//print_r($_GET['art_id']);exit();
		$str = "Location:http://zhengjinnet.com/article-".$_GET['art_id'].".html";
		Header($str);
	}
	...
	/* 获取用户中心推荐产品列表 （is_hot）*/ 
    $sql = 'SELECT g.*, c.cat_name FROM ' . $GLOBALS['ecs']->table('goods') . " AS g INNER JOIN " . $ecs->table("category") . " AS c ON g.cat_id=c.cat_id WHERE g.is_hot = 1 AND g.goods_start_time < ".time()." AND g.goods_end_time > ".time(). " AND g.goods_rest_number > 0";
	$goods_list = $GLOBALS['db']->getAll($sql);
	$smarty->assign('goods_list', $goods_list);	
	
	/* 获取用户信息 */
	$sql = 'SELECT user_name,is_bank_card,is_id_card FROM ' . $ecs->table("users") . "WHERE user_id = " . $_SESSION["user_id"];
	$user = $GLOBALS['db']->getRow($sql);
	$smarty->assign('is_id_card',$user["is_id_card"]);
	$smarty->assign('is_bank_card',$user["is_bank_card"]);
	...
	// 计算用户总资产
	//$total_money = $total_inverst_money + $rest_earn_money + $user["user_money"];
	
	/* 保留两位小数 */
	/* $formed_total_money = round($total_money*100)/100;
	$formed_total_inverst_money = round($total_inverst_money*100)/100;
	$formed_total_earn_money = round($total_earn_money*100)/100;
	$formed_rest_earn_money = round($rest_earn_money*100)/100;
	$formed_taking_earn_money = round($taking_earn_money*100)/100;
	
	$smarty->assign('formed_total_money', $formed_total_money);
	$smarty->assign('formed_total_inverst_money', $formed_total_inverst_money);	
	$smarty->assign('formed_total_earn_money', $formed_total_earn_money);
	$smarty->assign('formed_rest_earn_money', $formed_rest_earn_money);	
	$smarty->assign('formed_taking_earn_money', $formed_taking_earn_money);		 */
	
	
	/* 保留两位小数 */
	$formed_total_money = round($buy_info["total_money"]*100)/100;
	$formed_total_inverst_money = round($buy_info["total_inverst_money"]*100)/100;
	$formed_total_earn_money = round($buy_info["total_earn_money"]*100)/100;
	$formed_rest_earn_money = round($buy_info["rest_earn_money"]*100)/100;
...
if ($action == 'identify')
{
	...
	/* 获取用户信息 */
	$sql = 'SELECT user_id, user_name, user_money, is_bank_card, is_id_card, true_name, id_number, fuiou_bank_nm, fuiou_capacntno, q1,q2,q3,q4,q5,is_form FROM ' . $ecs->table("users") . "WHERE user_id = " . $_SESSION["user_id"];
	$user_list = $GLOBALS['db']->getAll($sql);
	$user = $user_list[0];	
	if ($user["is_id_card"]){
		
		$head = substr(strval($user["id_number"]),0,4);
		$tail = substr(strval($user["id_number"]),-3,3);
		$formed_id_number = $head."***********".$tail;
		$user["formed_id_number"] = $formed_id_number;
	}
	if ($user["is_bank_card"]){
		
		$head = substr(strval($user["fuiou_capacntno"]),0,6);
		$tail = substr(strval($user["fuiou_capacntno"]),-4,4);
		$mid = "";
		for ($i=0;$i<strlen($user["fuiou_capacntno"]);$i++){
			$mid .= "*";
		}
		$formed_fuiou_capacntno = $head.$mid.$tail;
		$user["formed_fuiou_capacntno"] = $formed_fuiou_capacntno;
	}	
	if ($user["is_form"]){
		$total_point = 0;
		switch ($user["q1"])
		...
		if ($total_point < 20){
			$user["user_form_comment"] = "低风险承受能力类E";
		}elseif($total_point < 60){
			$user["user_form_comment"] = "中等风险承受能力类C";
		}elseif($total_point < 80){
			$user["user_form_comment"] = "中高等风险承受能力类B";
		}else{
			$user["user_form_comment"] = "高等风险承受能力类A";
		}
	}
	$user["formed_mobile"] = substr(strval($user["user_name"]),0,4)."****".substr(strval($user["user_name"]),-3,3);
...
if ($action == 'recharge')
{
	...
	$smarty->assign('is_id_card',$is_id_card);
	$smarty->assign('is_bank_card',$is_bank_card);
    $smarty->display('user_clips.dwt');
}
// 余额提现页面
if ($action == 'withdraw')
{
	...
	$user_money = get_user_money($_SESSION["user_id"],$user["user_name"]);
	$user["user_money"] = $user_money["ca_balance"];
	$user["format_user_money"] = $user_money["format_ca_balance"];
	$smarty->assign('user',$user);
    $smarty->display('user_clips.dwt');
}
// 资金流水页面
...
{
	/* 获取用户信息 */
	$sql = 'SELECT user_name,user_money,is_id_card,is_bank_card FROM ' . $ecs->table("users") . "WHERE user_id = " . $_SESSION["user_id"];
	$user = $GLOBALS['db']->getRow($sql);
	$smarty->assign('is_id_card',$user["is_id_card"]);
	$smarty->assign('is_bank_card',$user["is_bank_card"]);
	require_once(ROOT_PATH .'includes/lib_fuiou.php');
	...
	$buy_info = get_user_order_total($_SESSION["user_id"], $user_money["ca_balance"]);
	// 计算用户总资产
	$total_money = $buy_info['total_money'];//总资产
	...
	/* 获取用户资金流水列表 */
	/* 旧版本调用本地money_log 数据库 */
/*	$sql = 'SELECT * FROM ' . $ecs->table("money_log") . "WHERE user_id = " . $_SESSION["user_id"] . " ORDER BY log_time DESC";
	$money_list = $GLOBALS['db']->getAll($sql);
	foreach ($money_list as $key => $val){
		$money_list[$key]["formed_log_time"] = date("Y.m.d",$val["log_time"]);
	}
	$smarty->assign('money_list',$money_list);*/
	/* 新版本同步富友数据库 */

	$smarty->assign('user',$user);
	if ($_POST["select_year"]||$_POST["select_month"]){
		$select_year = $_POST["select_year"];
		$select_month = $_POST["select_month"];
	}else{
		$select_year = date("Y",time());
		$select_month = date("m",time());
	}
	
	$user_log = array();
	// 获取交易信息
	// 获取31天前的日期
	$day_31 = local_date("d",(gmtime()-31*24*60*60));
	$day_32 = local_date("d",(gmtime()-32*24*60*60));
	$month_31 = local_date("m",(gmtime()-31*24*60*60));
	$year_31 = local_date("Y",(gmtime()-31*24*60*60));
	// 判断 如果查询月是上一个月的话
	if (($month_31 == $select_month)&&($year_31==$select_year)){
		$money_list_1a = get_money_log($_SESSION["user_id"],$select_year.$select_month."01",$select_year.$select_month.$day_32);
		$money_list_1b = get_money_log($_SESSION["user_id"],$select_year.$select_month.$day_31,$select_year.$select_month."31");
		$money_list = array_merge($money_list_1a,$money_list_1b);
		$money_list_2a = get_money_log2($_SESSION["user_id"],$select_year.$select_month."01",$select_year.$select_month.$day_32);
		$money_list_2b = get_money_log2($_SESSION["user_id"],$select_year.$select_month.$day_31,$select_year.$select_month."31");
		$money_list2 = array_merge($money_list_2a,$money_list_2b);
	}else{
		$money_list = get_money_log($_SESSION["user_id"],$select_year.$select_month."01",$select_year.$select_month."31");
		$money_list2 = get_money_log2($_SESSION["user_id"],$select_year.$select_month."01",$select_year.$select_month."31");
	}
	
	foreach ($money_list as $key => $val){
		$money_list[$key]["format_txn_date"] = substr($val["txn_date"],0,4)."-".substr($val["txn_date"],4,2)."-".substr($val["txn_date"],6,2);
		$money_list[$key]["format_txn_time"] = substr($val["txn_time"],0,2).":".substr($val["txn_time"],2,2).":".substr($val["txn_time"],4,2);
		$sql = "SELECT goods_name FROM " . $ecs->table('goods') . " WHERE fuiou_id = '". $val["in_cust_no"]."'";
		$money_list[$key]["goods_name"] = $db->getOne($sql);
		if ($money_list[$key]["in_cust_no"] == $user["user_name"]){
			$money_list[$key]["type"] = "还款";
			$money_list[$key]["remark"] = "还款";
			$money_list[$key]["sign"] = 1;
		}else{
			$money_list[$key]["type"] = "手续费";
			$money_list[$key]["remark"] = "手续费";
			$money_list[$key]["sign"] = 0;
		}
		$money_list[$key]["format_txn_amt"] = number_format($val["txn_amt"]/100,2);
		$money_list[$key]["time"] = mktime(substr($val["txn_time"],0,2),substr($val["txn_time"],2,2),substr($val["txn_time"],4,2),substr($val["txn_date"],4,2),substr($val["txn_date"],6,2),substr($val["txn_date"],0,4));
	}

	foreach ($money_list2 as $key => $val){
		$ssn = $money_list2[$key]["mchnt_ssn"];
		$code = substr($ssn,0,4);
		
		if ($code=="4003"){
			$money_list2[$key]["format_txn_date"] = substr($val["txn_date"],0,4)."-".substr($val["txn_date"],4,2)."-".substr($val["txn_date"],6,2);
			$money_list2[$key]["format_txn_time"] = substr($val["txn_time"],0,2).":".substr($val["txn_time"],2,2).":".substr($val["txn_time"],4,2);
			$sql = "SELECT goods_name FROM " . $ecs->table('goods') . " WHERE fuiou_id = '". $val["in_cust_no"]."'";
			$money_list2[$key]["goods_name"] = $db->getOne($sql);
			$money_list2[$key]["type"] = "产品购买";
			$money_list2[$key]["sign"] = 0;
			$money_list2[$key]["format_txn_amt"] = number_format($val["txn_amt"]/100,2);
			$money_list2[$key]["time"] = mktime(substr($val["txn_time"],0,2),substr($val["txn_time"],2,2),substr($val["txn_time"],4,2),substr($val["txn_date"],4,2),substr($val["txn_date"],6,2),substr($val["txn_date"],0,4));
		}else{
			$money_list2[$key]["format_txn_date"] = substr($val["txn_date"],0,4)."-".substr($val["txn_date"],4,2)."-".substr($val["txn_date"],6,2);
			$money_list2[$key]["format_txn_time"] = substr($val["txn_time"],0,2).":".substr($val["txn_time"],2,2).":".substr($val["txn_time"],4,2);
			if ($money_list2[$key]["in_cust_no"] == $user["user_name"]){
				$money_list2[$key]["type"] = "转让产品出售";
				$money_list2[$key]["sign"] = 1;
			}else{
				$money_list2[$key]["type"] = "转让产品购买";
				$money_list2[$key]["sign"] = 0;
			}
			$money_list2[$key]["format_txn_amt"] = number_format($val["txn_amt"]/100,2);
			$money_list2[$key]["time"] = mktime(substr($val["txn_time"],0,2),substr($val["txn_time"],2,2),substr($val["txn_time"],4,2),substr($val["txn_date"],4,2),substr($val["txn_date"],6,2),substr($val["txn_date"],0,4));	
	...
	foreach ($money_list2 as $val){
		$user_log[] = $val;
	}

	// 获取充值提现信息
	$recharge_list = get_recharge_log($_SESSION["user_id"],"PW11",$select_year."-".$select_month."-01 00:00:00",$select_year."-".$select_month."-31 23:59:59");
	foreach ($recharge_list as $key => $val){
		$recharge_list[$key]["format_txn_date"] = substr($val["txn_date"],0,4)."-".substr($val["txn_date"],4,2)."-".substr($val["txn_date"],6,2);
		$recharge_list[$key]["format_txn_time"] = substr($val["txn_time"],0,2).":".substr($val["txn_time"],2,2).":".substr($val["txn_time"],4,2);
		$recharge_list[$key]["remark"] = "充值";
		$recharge_list[$key]["type"] = "充值";
		$recharge_list[$key]["sign"] = 1;
		$recharge_list[$key]["format_txn_amt"] = number_format($val["txn_amt"]/100,2);
		$recharge_list[$key]["time"] = mktime(substr($val["txn_time"],0,2),substr($val["txn_time"],2,2),substr($val["txn_time"],4,2),substr($val["txn_date"],4,2),substr($val["txn_date"],6,2),substr($val["txn_date"],0,4));
	}	
	// 插入user_log
	foreach ($recharge_list as $val){
		$user_log[] = $val;
	}
	
	$withdraw_list = get_recharge_log($_SESSION["user_id"],"PWTX",$select_year."-".$select_month."-01 00:00:00",$select_year."-".$select_month."-31 23:59:59");
	foreach ($withdraw_list as $key => $val){
		$withdraw_list[$key]["format_txn_date"] = substr($val["txn_date"],0,4)."-".substr($val["txn_date"],4,2)."-".substr($val["txn_date"],6,2);
		$withdraw_list[$key]["format_txn_time"] = substr($val["txn_time"],0,2).":".substr($val["txn_time"],2,2).":".substr($val["txn_time"],4,2);
		$withdraw_list[$key]["remark"] = "提现";
		$withdraw_list[$key]["type"] = "提现";
		$withdraw_list[$key]["sign"] = 1;
		$withdraw_list[$key]["format_txn_amt"] = number_format($val["txn_amt"]/100,2);
		$withdraw_list[$key]["time"] = mktime(substr($val["txn_time"],0,2),substr($val["txn_time"],2,2),substr($val["txn_time"],4,2),substr($val["txn_date"],4,2),substr($val["txn_date"],6,2),substr($val["txn_date"],0,4));
	}	
	// 插入user_log
	...
	array_multisort($time_mark, SORT_DESC, $user_log);
	
	$smarty->assign('money_list',$user_log);
	$smarty->assign('recharge_list',$recharge_list);
	//echo "<pre>";print_r($money_list);exit;
    $smarty->display('user_clips.dwt');
}

// 验证身份证实名认证信息
if ($action == 'act_identify_id_card'){
	include_once(ROOT_PATH .'includes/lib_clips.php');
	...
	if (empty($true_name)){
		show_message("请输入您的真实姓名");
	}
	if (empty($id_number)){
		show_message("请输入您的真实身份证号码");
	}
	if (empty($id_number2)){
		show_message("请输入您的真实身份证号码");
	}
	if ($id_number != $id_number2){
		show_message("两次输入的身份证号码不一致，请确认后重新输入");
	}
	$sql = 'SELECT count(*) FROM ' . $ecs->table('users') . " WHERE true_name= '". $true_name." '";   
    $if_exist = $db->getOne($sql);	
	if ($if_exist){
		show_message("该用户已成为政金网实名认证用户，请确认后重新输入");
	}
	$mall_id='110294';
	$appkey='fd7a8ac2e9b6f7713766384258e5919b';
	$url='http://121.41.42.121:8080/v2/id-server?';
	
	idcard_verify($true_name,$id_number);

}
//我的投资
if ($action == 'user_product')
{
	if (($_POST["flag"]=="transfer")&&isset($_POST["order_id"])){
		$sql = 'SELECT transfer_flag FROM ' . $ecs->table("order_new") . " WHERE user_id = " . $_SESSION["user_id"] ." AND order_id = ".$_POST["order_id"];
		$order = $GLOBALS['db']->getRow($sql);
		if (!$order){
			show_message("非法入口，订单不存在。",array("返回首页", "前往用户中心"), array("index.html", 'user.php'), 'info',false);
		}
		if ($order["transfer_flag"]==1){
			show_message("重复操作！订单已在转让中。",array("返回首页", "前往用户中心"), array("index.html", 'user.php'), 'info',false);
		}
		if ($order["transfer_flag"]==2){
			show_message("非法入口，订单已经被转让。",array("返回首页", "前往用户中心"), array("index.html", 'user.php'), 'info',false);
		}
        $sql = 'UPDATE ' . $ecs->table('order_new') . " SET `transfer_flag`=1, `transfer_start_time`= ".time()." , `transfer_amount`= '".$_POST["transfer_amount"]."' WHERE `order_id`='" . $_POST["order_id"] . "'";
        $db->query($sql);

		show_message("操作成功，订单已转至转让队列。",array("返回首页", "前往用户中心"), array("index.html", 'user.php'), 'info',false);
	}
    include_once(ROOT_PATH .'includes/lib_clips.php');

	$filter = $_GET["filter"]?$_GET["filter"]:1;
	$smarty->assign('filter', $filter);
	
	/* 获取用户信息 */
	$sql = 'SELECT user_name,user_money,is_bank_card FROM ' . $ecs->table("users") . " WHERE user_id = " . $_SESSION["user_id"];
	$user_list = $GLOBALS['db']->getAll($sql);
	$user = $user_list[0];	
	
	/* 获取用户的购买信息 */
    $sql = 'SELECT o.*,g.cat_id,g.goods_transfer_flag, g.goods_name, g.goods_start_time, g.goods_period, g.goods_interest_rate, g.goods_earn_method, g.t  FROM ' . $GLOBALS['ecs']->table('order_new') . " AS o INNER JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON o.goods_id = g.goods_id WHERE o.user_id = " . $_SESSION["user_id"];
	...
	foreach ($order_list as $key => $val){
		...
		if ($val["transfer_flag"] == 2){
			
				$order_list[$key]["taking_day"] = time_to_day($val["transfer_end_time"]-$val["pay_time"]);
				$order_list[$key]["formed_transfer_end_time"] = date("Y-m-d",$val["transfer_end_time"]);
				...
				$order_list[$key]["order_status"] = 4;
				
				$order_list[$key]["is_return"] = 0;
 			...
 			if ($val["parent_id"]){	
				...
				if ($val["goods_earn_method"] == 1){
					...
					$total_earn_money += $total_earn_money_0;
				}
				...
				if ($val["goods_earn_method"] == 3){
					...
					$sql='select transfer_time from'.$ecs->table('order_new').' where parent_id='.$val['order_id'];
					$transfer_time=$db->getOne($sql);	
					$old_get_paid=floor(time_to_day($transfer_time-$val['pay_time']+$goods['t'])/30);
					$new_get_paid=floor(time_to_day($val['transfer_time']-$val['pay_time']+$goods['t'])/30);
					$day_number=$old_get_paid-$new_get_paid;
				...
				if ($val["goods_earn_method"] == 4){
					...
					$sql='select transfer_time from'.$ecs->table('order_new').' where parent_id='.$val['order_id'];
					$transfer_time=$db->getOne($sql);	
					$old_get_paid=floor(time_to_day($transfer_time-$val['pay_time']+$goods['t'])/90);
					$new_get_paid=floor(time_to_day($val['transfer_time']-$val['pay_time']+$goods['t'])/90);
					$day_number=$old_get_paid-$new_get_paid;
					$total_earn_money_0 = $val["amount"] * $val["goods_interest_rate"] * $day_number /36500;
					$total_earn_money += $total_earn_money_0;
				}
				$transfer_win_0 = $val["transfer_amount"] - $val["buy_transfer_money"];
				...
				if ($val["goods_earn_method"] == 3){
					//按月收益，到期回本
					//计算该项目得到的收益，未结清项目计算至当前日期
					$day_number = floor((time_to_day($val["transfer_end_time"])-time_to_day($val["pay_time"]))/30)*30;
					$total_earn_money_0 = $val["amount"] * $val["goods_interest_rate"] * $day_number /36500;
					$total_earn_money += $total_earn_money_0;
				...
				if ($val["goods_earn_method"] == 4){
					//按季收益，到期回本
					//计算该项目得到的收益，未结清项目计算至当前日期
					$day_number = floor((time_to_day($val["transfer_end_time"])-time_to_day($val["pay_time"]))/90)*90;
					$total_earn_money_0 = $val["amount"] * $val["goods_interest_rate"] * $day_number /36500;
					$total_earn_money += $total_earn_money_0;
				}
				$transfer_win_0 = $val["transfer_amount"] - $val["amount"];		
			}
			$total_earn_money += $transfer_win_0;
			$order_list[$key]["total_earn_money"] = round($transfer_win_0*100)/100;
			//var_dump($total_earn_money_0);
			...
			if ($val["parent_id"]){
				if ($val["pay_time"]>0){
					...
					if ($goods_return_time_0 > time()){
						...
						$order_list[$key]["is_return"] = 0;
						// 计算正在投资的金额，即还未结清的项目总额
						$total_inverst_money += $val["buy_transfer_amount"];
						...
						if ($val["goods_earn_method"] == 3){
							...
							$orignial_earn_money_0 = $val["amount"] * $val["goods_interest_rate"] * $day_number2 /36500;
							$total_earn_money += $order_earn_money_0;
						...
						if ($val["goods_earn_method"] == 4){
							...
							$day_number2 = floor((time_to_day($val["transfer_time"])-time_to_day($val["pay_time"]))/90)*90;
							$day_number = $day_number1 - $day_number2;
						...
						$order_list[$key]["rest_earn_money"] = round($rest_earn_money_0*100)/100;
						$order_list[$key]["total_earn_money"] = round($total_earn_money_0*100)/100;
						...
						$taking_earn_money += $taking_earn_money_0;

						$order_list[$key]["next_earn_money"] = round($next_earn_money_0*100)/100;
						if ($val["parent_id"]){
							if ((time_to_day($next_earn_time_0)-time_to_day(time())>5)
								...
								$order_list[$key]["can_transfer"]=1;
							}else{
								/* var_dump(date('Y-m-d',$last_earn_time_0));
								var_dump(date('Y-m-d',time()));echo '当前时间';
								var_dump(date('Y-m-d',$next_earn_time_0));echo '下一次收益时间';
								var_dump(date('Y-m-d',$val["transfer_time"]));echo '转让时间'; */
								$order_list[$key]["can_transfer"]=0;
							}
						}else{
							if ((time_to_day($next_earn_time_0)-time_to_day(time())>5)
						...
						$order_list[$key]["formed_next_earn_time"] = date("Y.m.d",$next_earn_time_0);
						$order_list[$key]["taking_earn_money"] = round($taking_earn_money_0*100)/100;
						$order_list[$key]["taking_earn_money_and_amount"] = round($taking_earn_money_0*100)/100 + $val["amount"];
					}else{
						
						$order_list[$key]["order_status"] = 2;
						//已结清
						$order_list[$key]["is_return"] = 1;
						// 计算已收总收益，即该项目总收益
						$total_earn_money_0 =$all_earn_0;
						$order_list[$key]["total_earn"] = round($all_earn_0*100)/100;
						$order_list[$key]["total_get"] = round(($all_earn_0 + $val["amount"])*100)/100;
						$total_earn_money += $total_earn_money_0;
					}
				}else{
					$order_list[$key]["order_status"]=3;
				}				
			}else{
				//正常用户
				if ($val["pay_time"]>0){
					...
					if ($goods_return_time_0 > time()){
						...
						if ($val["goods_earn_method"] == 4){
							...
							//计算下一次收益时间
							$next_earn_time_0 = $val["pay_time"];
							while ($next_earn_time_0+$val['t']*24*3600<time()){
								$next_earn_time_0 += 90*24*60*60;
							}
							$last_earn_time_0 = $next_earn_time_0 - 90*24*60*60;
...
elseif ($action == 'act_register')
{
    ...
    else
    {
       ...
       if($enabled_sms){
		...
		//print_r($_POST['seller_no']);
		if(empty($_POST['seller_no']))
        {
            show_message("请填写业务员编号");
        }
		$sql = 'SELECT seller_id FROM ' . $ecs->table('seller') . ' WHERE seller_no = "'.$_POST["seller_no"].'"';   
        $seller_id = $db->getOne($sql);
		//print_r($seller_id);
		if(!$seller_id)
        {
            show_message("业务员编号不存在");
        }		
        if(empty($_POST['agreement']))
        {
            show_message($_LANG['passport_js']['agreement']);
        }
        if(!$_SESSION['sms_mobile_code'])
        {
            show_message("点击获取短信验证码");
        }
        ...
        {
            show_message("登录密码不能少于 8 个字符");
        }
        if (strlen($password) > 20)
        {
            show_message("密码长度不得超过20位");
        }
        ...
        {
			// 将业务员编码写入用户数据库
			$sql = 'UPDATE ' . $ecs->table('users') . " SET `seller_id`='$seller_id' WHERE `user_id`='" . $_SESSION['user_id'] . "'";
            $db->query($sql);
			// 更新为管理员模式
...
/* 通过手机号修改绑定手机的处理 */
elseif ($action == 'act_change_mobile')
{
       include_once(ROOT_PATH . 'includes/lib_passport.php');
        
       $username = $other['mobile_phone'] = isset($_POST['mobile_phone']) ? trim($_POST['mobile_phone']) : '';
       ...
       $email    = $username .'@qq.com';
        ...
        //注册类型 by carson end
        $sql = 'SELECT count(*) FROM ' . $ecs->table('users') . ' WHERE user_name= '. $_POST["mobile_phone"];   
        $if_user = $db->getOne($sql);	

		if ($if_user){
			show_message("该用户已存在，请重新输入");
		}
        if(!$_SESSION['sms_mobile_code'])
        {
            show_message("点击获取短信验证码");
        ...
        if($_POST['mobile_phone']!=$_SESSION['sms_mobile'])
        {
            show_message("手机号码与获取验证码的号码不符");
        }
        if($_POST['mobile_code']!=$_SESSION['sms_mobile_code'])
        ...
        if ($user->check_user($_SESSION["user_name"], $password) <= 0)
        {
            show_message("您输入的密码不正确，请重新输入");
        }
		...
		$pwd = $user-> compile_password(array('password'=>$password));
            $sql = "UPDATE " . $ecs->table('users') . " SET user_name = '$username', email = '$email', mobile_phone = '$username' WHERE user_name= '".$_SESSION["user_name"]."'";
            $db->query($sql);
		$user->logout();
		show_message("成功修改绑定手机",array("返回首页","前往重新登陆"),array("index.html","user.php"));

}
/* 通过手机号修改密码的处理 */
elseif ($action == 'act_get_password_m')
{
     /* 增加是否关闭注册 */
    if ($_CFG['shop_reg_closed'])
    {
    ...
    else
    {
       ...
       if($enabled_sms){
        ...
        //注册类型 by carson end
        $back_act = isset($_POST['back_act']) ? trim($_POST['back_act']) : '';
        $sql = 'SELECT count(*) FROM ' . $ecs->table('users') . ' WHERE user_name= '. $_POST["mobile"];   
        $if_user = $db->getOne($sql);	

		if (!$if_user){
			show_message("用户名不存在，请确认后重新输入");
		}
        if(!$_SESSION['sms_mobile_code'])
        {
            show_message("点击获取短信验证码");
		...
		$pwd = $user-> compile_password(array('password'=>$password));
            $sql = "UPDATE " . $ecs->table('users') . " SET password = '$pwd', ec_salt = 0 WHERE user_name= '".$_POST["mobile"]."'";
            $db->query($sql);		
		show_message("成功修改密码",array("返回首页","立刻前往登陆"),array("index.html","user.php"));
    }
}
/* 提交风险评估表单 */
elseif ($action == 'complete_form')
{
	if (!$_POST["q1"]||!$_POST["q2"]||!$_POST["q3"]||!$_POST["q4"]||!$_POST["q5"]){
		show_message("请完整填写表单后进行提交。");
	}
	$q1 = $_POST["q1"];
	$q2 = $_POST["q2"];
	$q3 = $_POST["q3"];
	$q4 = $_POST["q4"];
	$q5 = $_POST["q5"];
	$sql = "UPDATE " . $ecs->table('users') . " SET q1 = ".$q1.", q2 = ".$q2.", q3 = ".$q3.", q4 = ".$q4.", q5 = ".$q5.", is_form = 1 WHERE user_id=".$_SESSION["user_id"];
    $db->query($sql);
	ecs_header("Location: user.php?act=identify\n");
    exit;
}
/* 验证用户注册邮件 */
...
elseif ($action == 'login')
{
	$flag = $_GET['login_status'];
	session_start();
	$_SESSION['login_status'] = $flag;
...
elseif ($action == 'act_login')
{
	...
	if ($seller_no){
		$sql = 'SELECT u.seller_id FROM ' . $ecs->table("users") . " AS u LEFT JOIN ".$ecs->table("seller")." AS s ON s.seller_id = u.seller_id WHERE s.seller_no = '" . $seller_no."'";
		$seller_id = $GLOBALS['db']->getOne($sql);
		
		$sql = 'SELECT s.seller_mac FROM ' . $ecs->table("users") . " AS u LEFT JOIN ".$ecs->table("seller")." AS s ON s.seller_id = u.seller_id WHERE s.seller_no = '" . $seller_no."'";
		$mac_num = $GLOBALS['db']->getOne($sql);
		$sql = 'SELECT mac FROM ' . $ecs->table('mac_list');
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
		...
		if($_SESSION['login_status']==1){
			Header("Location:category.php?id=5&project=3");
		}elseif($_SESSION['login_status']==2){
			$str = "Location:http://zhengjinnet.com/article-".$_SESSION['art_id'].".html";
			Header($str);
			//Header("Location:http://zhengjinnet.com/goods.php?id=123");
		}else{
...
elseif ($action == 'logout')
{
    if ((!isset($back_act)|| empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
    {
        $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.html') ? './index.html' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    }

    $user->logout();
    $ucdata = empty($user->ucdata)? "" : $user->ucdata;
    show_message($_LANG['logout'] . $ucdata, array($_LANG['back_up_page'], $_LANG['back_home_lnk']), array($back_act, 'index.html'), 'info');
}
...
{
	/* 获取用户信息 */
	$sql = 'SELECT user_name,user_money,is_bank_card,is_id_card,true_name,id_number FROM ' . $ecs->table("users") . "WHERE user_id = " . $_SESSION["user_id"];
	$user_list = $GLOBALS['db']->getAll($sql);
	$user = $user_list[0];	
...
elseif ($action =='email_list')
{
    ...
    elseif ($job == 'add_check')
    {
        ...
        show_message($info, $_LANG['back_home_lnk'], 'index.html');
    }
