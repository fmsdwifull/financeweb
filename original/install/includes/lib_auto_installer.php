...
function do_others($system_lang, $captcha, $goods_types, $install_demo, $integrate_code)
{
    ...
    if (intval($install_demo))
    {
        ...
        if (!copy_files(ROOT_PATH . 'demo/201410/goods_img/', ROOT_PATH . 'images/201410/goods_img/'))
        {
            $err->add(implode('', $err->last_message()));
            return false;
        }
        if (!copy_files(ROOT_PATH . 'demo/201410/thumb_img/', ROOT_PATH . 'images/201410/thumb_img/'))
        {
            $err->add(implode('', $err->last_message()));
            return false;
        }
        if (!copy_files(ROOT_PATH . 'demo/201410/source_img/', ROOT_PATH . 'images/201410/source_img/'))
        {
            $err->add(implode('', $err->last_message()));
            return false;
        }
