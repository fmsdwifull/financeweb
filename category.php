...
//print_r(dirname(__FILE__) . '/includes/init.php');
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    foreach ($res as $row)
    {
		...
		$arr[$row['goods_id']]['goods_rest_number'] = $row['goods_total_number'] - $sold_number  ;
		...
		if (time()>($row['goods_start_time']+$row['goods_period']*24*60*60)){
			...
			if (time()>($row['goods_end_time'])){
				...
				if ($arr[$row['goods_id']]['goods_rest_number']<=0){
